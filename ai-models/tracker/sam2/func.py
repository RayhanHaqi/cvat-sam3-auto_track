...
class _Sam2Tracker:
    ...
    spec = cvataa.TrackingFunctionSpec(supported_shape_types=["mask", "polygon", "rectangle"])
    ...
    def _shape_to_mask(
        ...
        if shape.type == "rectangle":
            mask = np.zeros((pp_image.original_height, pp_image.original_width), dtype=np.uint8)
            x1, y1, x2, y2 = (int(v) for v in shape.points[:4])
            mask[y1:y2, x1:x2] = 1
            return mask
    ...
    def _mask_to_shape(
        ...
        if context.original_shape_type == "rectangle":
            mask_np = np.asarray(mask, dtype=np.uint8)
            ...
            return cvataa.TrackableShape(type="rectangle", points=[x1, y1, x2, y2])
