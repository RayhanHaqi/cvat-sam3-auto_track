...
# This script runs the OSTrack-384 model agent in CVAT.
...
echo "Running OSTrack-384 function agent for FUNCTION_ID: $FUNCTION_ID..."

exec cvat-cli --server-host "$CVAT_BASE_URL" "${ORG_SLUG_ARGS[@]}" function run-agent "$FUNCTION_ID" --function-file="$FUNCTION_FILE_PATH" "${USE_CUDA_ARGS[@]}"
