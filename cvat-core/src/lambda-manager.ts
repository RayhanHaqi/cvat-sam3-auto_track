...
import {
    estimateJsonBytes,
    extractAutoTrackDiagMeta,
    getAutoTrackDiagBridge,
} from './auto-track-diagnostics-bridge';
import {
...
class LambdaManager {
    ...
    async call(taskID, model, args): Promise<TrackerResults | InteractorResults | SerializedCollection> {
        ...
        const bridge = getAutoTrackDiagBridge();
        const diagMeta = extractAutoTrackDiagMeta(body);
        const requestPayloadBytes = estimateJsonBytes(body);
        const lambdaStart = typeof performance !== 'undefined' ? performance.now() : Date.now();
        if (bridge?.isEnabled() && diagMeta) {
            bridge.onLambdaRequestStart({
                ...diagMeta,
                requestPayloadBytes,
            });
        }

        let result: TrackerResults | InteractorResults | SerializedCollection | undefined;
        try {
            result = await serverProxy.lambda.call(model.id, body);
        } finally {
            if (bridge?.isEnabled() && diagMeta) {
                const durationMs = (typeof performance !== 'undefined' ? performance.now() : Date.now()) - lambdaStart;
                bridge.onLambdaRequestEnd({
                    ...diagMeta,
                    durationMs,
                    requestPayloadBytes,
                    responsePayloadBytes: result !== undefined ? estimateJsonBytes(result) : null,
                });
            }
        }
