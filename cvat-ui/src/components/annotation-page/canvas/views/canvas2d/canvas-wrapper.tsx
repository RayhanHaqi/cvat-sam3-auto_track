...
import { autoTrackDiagnostics } from 'utils/auto-track-diagnostics';
import {
...
class CanvasWrapperComponent extends React.PureComponent<Props> {
    ...
    private updateCanvas(): void {
        ...
        if (frameData !== null && canvasInstance) {
            ...
            const setupStart = typeof performance !== 'undefined' ? performance.now() : Date.now();
            if (autoTrackDiagnostics.isEnabled()) {
                autoTrackDiagnostics.emit('canvas_setup_start', {
                    frame,
            ...
            if (autoTrackDiagnostics.isEnabled()) {
                const setupMs = (typeof performance !== 'undefined' ? performance.now() : Date.now()) - setupStart;
                const visibleShapeCount = frameData.deleted ? 0 : filteredAnnotations.length;
                autoTrackDiagnostics.recordCanvasSetup(setupMs, {
                    visibleShapeCount,
                    canvasObjectCount: visibleShapeCount,
                    totalVisibleCanvasShapeCount: visibleShapeCount,
                });
            }
