...
def _auto_track_diag_enabled() -> bool:
    return os.environ.get("AUTO_TRACK_DIAGNOSTICS", "").strip().lower() in ("1", "true", "yes")


def _auto_track_diag_log(event_name: str, **fields: Any) -> None:
    if not _auto_track_diag_enabled():
        return
    diag_meta = fields.pop("diagMeta", None)
    correlation: dict[str, Any] = {}
    if isinstance(diag_meta, dict):
        correlation = {
    ...
    record = {"event": event_name, **correlation, **fields}
        ...
        *,
        ...
        if self.kind == FunctionKind.DETECTOR:
            ...
            try:
                ...
                if self.id == SAM3_TRACKER_FUNCTION_ID:
                    ...
                    if "states" not in data:
                        ...
                        preload_fields = self._build_sam3_preload_fields(
                            db_task,
                            db_job,
                            frame_index,
                        )
        ...
        if _auto_track_diag_enabled() and self.kind == FunctionKind.TRACKER:
            import time

            request_payload_bytes = len(json.dumps(data, default=str))
        ...
        if _auto_track_diag_enabled() and self.kind == FunctionKind.TRACKER and invoke_started_at is not None:
            import time

            response_payload_bytes = len(json.dumps(response, default=str))
            _auto_track_diag_log(
                "lambda_request_end",
                functionId=self.id,
                requestType=data.get("type"),
                ...
                diagMeta=diag_meta,
                requestPayloadBytes=request_payload_bytes,
                responsePayloadBytes=response_payload_bytes,
                durationMs=(time.perf_counter() - invoke_started_at) * 1000.0,
            )
